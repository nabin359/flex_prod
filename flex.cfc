    ...
    produces="application/json"
{
  ...
  private numeric function getFiscalYearForDate(string dateStr) {
    var q = queryExecute(
            "SELECT TOP (1) [fiscal_year]
            FROM [PCSV_dw].[dbo].[Master_Calendar_Daily]
              where date_key = :dateStr;
            ",
            {
                dateStr = { value = dateStr, cfsqltype = "cf_sql_date" }
            },
            { datasource = "pcsv_dw" }
    );
    if(q.recordCount){
        return q.fiscal_year[1];
    } else {
        return 0;
    }
  }
  ...
  private struct function getStartAndEnd() {
    var today = DateFormat(now(), "yyyy-mm-dd");
        var result = {};
        var currentYear = getFiscalYearForDate(today) - 1;
  ...
  function post(
    ...
    try {
      // Validate no existing flex time request for same facility/craft/LDC/operationCode/date range
      var fiscalYear = getFiscalYearForDate( payload.wk_start );
      var q = queryExecute(
        "
          SELECT  mcd.fiscal_year
          FROM [PCSV_dw].[dbo].[Master_Calendar_Daily] as mcd
          inner join [pcsv_dw].[dbo].FlexTimeRequests as ftr
          on mcd.date_key = ftr.StartDate
          where mcd.fiscal_year = :fiscalYear
          and ftr.FacilityID = :facilityId
          and ftr.craft = :craft
          and ftr.LabourDistributionCode = :ldc
          and ftr.OperationNumber = :opnum;
        ",
        {
          fiscalYear : { value=fiscalYear,            cfsqltype="cf_sql_integer" },
      ...
      return { SUCCESS=false, MESSAGE="Failed to validate existing Flex time request." };
    }
   ...
   // writeLog(file="application", text="Flex post payload => " & serializeJSON(payload));
    ...
    // --- EMAIL: send receipt to the submitter (non-blocking) ---
    // try {
    //   sendAdminSubmitNotice( createdByVal, payload, result );
    // } catch (any mailErr) {
    //   writeLog(
    //     file = "application",
    //     text = "FlexService.post outer mail error: " & mailErr.message & " :: " & mailErr.detail
  ...
  private void function sendSubmitEmail(
    ...
    var toEmail       = "";
    var submitterName = "";
    if(arguments.payload.userName neq ""){
        submitterName = arguments.payload.userName;
    }  else if(arguments.createdByVal neq ""){
        submitterName = arguments.createdByVal;
    }
    ...
    try {
      ...
      if ( qMail.recordCount ) {
        ...
        if ( len(trim(qMail.userName[1])) > 0 && len(submitterName) == 0  )  { submitterName = trim(qMail.userName[1]); }
      }
