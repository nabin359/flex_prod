  ...
  data() {
    return {
      ...
      operationsList: [
        ...
        { ldc: 48, code: '7420', name: 'MIO Non Revenue Pickup' },
...
async notifyAfterSubmit() {
  try {
    ...
    async submitRequest() {
      ...
      if(this.user?.first_nm) userName = this.user.first_nm;
      if(this.user?.last_nm) userName += " " + this.user.last_nm;
...
'form.operation'(){ this.ensureOperationCodeValid(); },
  ...
  mounted() {
    ...
    document.getElementById('closeModal').addEventListener('click', async () => {
      ...
      window.location.href = 'https://eagnmnwbc1db.usps.gov/ref/toolbox/flex/dashboard.html';
    });
